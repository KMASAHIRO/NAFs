...
def test_net(rank, other_args):
    ...
    with torch.no_grad():
        ...
        for test_id in range(num_sample_test):
            ...
            mygt = gt.numpy()
            mygt_mag = mygt[:,:other_args.dir_ch].reshape(1, 1, dataset.sound_size[1], dataset.sound_size[2])
            mygt_phase = mygt[:,other_args.dir_ch:].reshape(1, 1, dataset.sound_size[1], dataset.sound_size[2])
