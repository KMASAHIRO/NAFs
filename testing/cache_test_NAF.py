    ...
    with torch.no_grad():
        for ori in [0, 1, 2, 3]:
            ...
            for test_id in range(num_sample_test):
                ...
                myout_mag = myout[...,0].reshape(1, 2, dataset.sound_size[1], dataset.sound_size[2])
                myout_phase = myout[...,1].reshape(1, 2, dataset.sound_size[1], dataset.sound_size[2])


                mygt = gt.numpy()
                mygt_mag = mygt[:,:2].reshape(1, 2, dataset.sound_size[1], dataset.sound_size[2])
                mygt_phase = mygt[:,2:].reshape(1, 2, dataset.sound_size[1], dataset.sound_size[2])
                # plt.imshow(np.concatenate([myout_phase[0][0], mygt_phase[0][0]], axis=-1))
                # plt.show()
                # exit()
                name = "{}_{}".format(ori, dataset.sound_name)
                ...
                # container["{}_{}".format(ori, dataset.sound_name)]= [myout_mag, myout_phase, mygt_mag, mygt_phase]
    # with open(save_name+"phase", "wb") as saver_file_obj:
