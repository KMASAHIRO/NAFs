...
def test_net(rank, other_args):
    ...
    auditory_net.eval()
    container = dict()
    save_name = os.path.join(other_args.result_output_dir, "MeshRIR_NAF.pkl")
    ...
    with torch.no_grad():
        ...
        for test_id in range(num_sample_test):
            ...
            myout_mag = myout[...,0].reshape(1, 1, dataset.sound_size[1], dataset.sound_size[2])
            myout_phase = myout[...,1].reshape(1, 1, dataset.sound_size[1], dataset.sound_size[2])
            mygt = gt.numpy()
            mygt_mag = mygt[:,:1].reshape(1, 1, dataset.sound_size[1], dataset.sound_size[2])
            mygt_phase = mygt[:,1:].reshape(1, 1, dataset.sound_size[1], dataset.sound_size[2])
