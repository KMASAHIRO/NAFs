...
def train_net(rank, world_size, freeport, other_args):
    ...
    for epoch in range(start_epoch, other_args.epochs+1):
        ...
        for data_stuff in sound_loader:
            ...
            try:
                output = ddp_auditory_net(total_in, degree, non_norm_position.squeeze(1)).transpose(1, 2)
                ...
                continue
            # output shape torch.Size([5, 2, 2000, 2])
            # gt shape torch.Size([5, 4, 2000])
            mag_loss = criterion(output[...,0], gt[:,:2])
            phase_loss = criterion(output[...,1], gt[:,2:]) * other_args.phase_alpha
