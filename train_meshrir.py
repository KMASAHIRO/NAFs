    ...
    for epoch in range(start_epoch, other_args.epochs+1):
        ...
        for data_stuff in sound_loader:
            ...
            try:
                ...
                continue
            # output shape torch.Size([5, 1, 2000, 2])
            # gt shape torch.Size([5, 2, 2000])
            mag_loss = criterion(output[...,0], gt[:,:1])
            phase_loss = criterion(output[...,1], gt[:,1:]) * other_args.phase_alpha
